...
function runTopLevel!(top_mod::anyModel,bendersCut_arr::Array{Dict{Tuple{Int64,Int64},bendersData},1},i::Int)
	...
	for bendersCut in bendersCut_arr
		...
		for sub in keys(bendersCut)
			...
			if haskey(top_mod.parts.exc.var, :capaExc)
...
function runSubLevel(sub_mod::anyModel,capaData_obj::bendersData; fixCapa::Bool = true, bounds::bendersData = bendersData())
	...
	elap = @elapsed optimize!(sub_mod.optModel)
	println("Finished $(sub_mod.subPro) in $(round(elap, digits=2)) s")
	...
	if haskey(sub_mod.parts.exc.cns, :ccbExc)
...
model_dir = "examples/testModel_stoch"

# create top level problem
top_mod = anyModel(model_dir, "results", objName = "top", reportLvl = 1)
set_optimizer(top_mod.optModel,Gurobi.Optimizer)
# set_optimizer_attribute(top_mod.optModel, "Method", 2)
# set_optimizer_attribute(top_mod.optModel, "Crossover", 0)
# set_optimizer_attribute(top_mod.optModel, "BarOrder", 1)
set_optimizer_attribute(top_mod.optModel, "LogToConsole", 0)

top_mod.subPro = tuple(0,0)
createOptModel!(top_mod)
setObjective!(:costs,top_mod)

nb_scens = length(values(top_mod.sets[:scr].nodes)) - 1
sub_tup = Tuple((1,x) for x in 1:nb_scens)
...
for (id,x) in enumerate(sub_tup)
    s = anyModel(model_dir,"results", objName = "sub_" * string(id), reportLvl = 1)
    s.subPro = x
    createOptModel!(s)
	...
	set_optimizer(s.optModel,Gurobi.Optimizer)
	set_optimizer_attribute(s.optModel, "Method", 2)
	set_optimizer_attribute(s.optModel, "Crossover", 0)
	#set_optimizer_attribute(s.optModel, "BarOrder", 0)
	set_optimizer_attribute(s.optModel, "LogToConsole", 0)
	# set_optimizer_attribute(s.optModel, "BarConvTol", 1e-3)
	#set_optimizer_attribute(s.optModel, "OptimalityTol", 1e-2)
...
let
	...
	while true
		...
		for x in sub_tup
			...
			if haskey(s.parts.exc.var, :ccbLowCapaExc)
				foreach(x -> fix(x,0.0; force = true), s.parts.exc.var[:ccbLowCapaExc][!,:var])
			end

			if haskey(s.parts.exc.var, :ccbUpCapaExc)
		...
		dual_etr = [Threads.@spawn runSubLevel(sub_dic[x],copy(capaData_obj)) for x in sub_tup]
		foreach(x-> push!(dual_dic, x[1] => fetch(x[2])), zip(sub_tup, dual_etr))
		...
		if i != 1
			...
			cost_df[!,:iteration] .= i#size(top_mod.parts.obj.cns[:bendersCuts],1)
...
# create top level problem
mono_mod = anyModel(model_dir, "results", objName = "mono", reportLvl = 1)
createOptModel!(mono_mod)
setObjective!(:costs,mono_mod)

set_optimizer(mono_mod.optModel,Gurobi.Optimizer)
set_optimizer_attribute(mono_mod.optModel, "Method", 2)
set_optimizer_attribute(mono_mod.optModel, "Crossover", 1)

optimize!(mono_mod.optModel)

reportResults(:summary,mono_mod)