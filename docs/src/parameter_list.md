...
!!! warning "Cost data before efficiency"
    Ensure the cost data provided relates to capacity **before efficiency** (see beginning of section)! Costs before efficiency are computed by multiplying costs after efficiency with a nominal efficiency ``K_{before} = K_{after} \cdot \eta``.
