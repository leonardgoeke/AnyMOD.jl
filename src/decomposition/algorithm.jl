...
# create subproblems in case of no distributed computing
function addSubproblems!(genSetup_ntup::NamedTuple{(:name, :frs, :supTsLvl, :shortExp, :threads, :opt), Tuple{String, Int64, Int64, Int64, Int64, DataType}},inputFolder_ntup::NamedTuple{(:in, :heu, :results), Tuple{Vector{String}, Vector{String}, String}},scale_dic::Dict{Symbol,NamedTuple},benders_obj::bendersObj,sub_tup::Tuple)

	# create sub-problems
	benders_obj.sub = Dict{Tuple{Int,Int},anyModel}()

	for (id,x) in enumerate(sub_tup)
		s = @suppress anyModel(inputFolder_ntup.in, inputFolder_ntup.result, objName = "subModel_" * string(id) * genSetup_ntup.name, lvlFrs = genSetup_ntup.frs, supTsLvl = genSetup_ntup.supTs, shortExp = genSetup_ntup.shortExp, coefRng = scale_dic[:rng], scaFac = scale_dic[:facSub], dbInf = benders_obj.options.dbInf, reportLvl = 1)
		s.subPro = x
		@suppress prepareMod!(s, genSetup_ntup.opt, genSetup_ntup.threads)
		set_optimizer_attribute(s.optModel, "Threads", genSetup_ntup.threads)
		benders_obj.sub[x] = s
	end
	...
	benders_obj.top.parts.obj.var[:cut] = map(y -> map(x -> y == 1 ? sub_tup[x][1] : sub_tup[x][2], 1:length(sub_tup)),1:2) |> (z -> createVar(DataFrame(Ts_dis = z[1], scr = z[2]),"subCut",NaN,benders_obj.top.optModel,benders_obj.top.lock,benders_obj.top.sets, scaFac = 1e2))
	push!(benders_obj.top.parts.obj.cns[:objEqn], (name = :aggCut, cns = @constraint(benders_obj.top.optModel, sum(benders_obj.top.parts.obj.var[:cut][!,:var]) == filter(x -> x.name == :benders, benders_obj.top.parts.obj.var[:objVar])[1,:var])))
