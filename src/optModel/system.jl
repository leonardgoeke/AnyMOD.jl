...
function addRetrofitting!(prepSys_dic::Dict{Symbol,Dict{Symbol,Dict{Symbol,NamedTuple}}},anyM::anyModel)
	...
	for sys in (:Te, :Exc)
		...
		for sSym in sysSym_arr, capaSym in filter(x -> occursin("capa",string(x)), intersect(collect(keys(allCapaDf_dic)),collect(keys(prepSys_dic[sys][sSym]))))
			...
			allRetroStart_df = adjustRetroRegion(sys,copy(allRetro_df))
			prepSys_dic[sys][sSym][retroName_sym] = (var = allRetroStart_df, resi = DataFrame()) 
			...
			for s in unique(allRetro_df[!,Symbol(sys,"_j")])
				prepSys_dic[sys][sysSym(s,anyM.sets[sys])][Symbol(:retro,type_sym)] = (var = orderDf(adjustRetroRegion(sys,filter(x -> x[Symbol(sys,"_j")] == s, allRetro_df),false)), resi = DataFrame())
			end
		end
	end
...
function removeFixed!(prepSys_dic::Dict{Symbol,Dict{Symbol,Dict{Symbol,NamedTuple}}},allCapaDf_dic::Dict{Symbol,DataFrame},anyM::anyModel)
	...
	for sys in (:Te,:Exc)
		...
		for sSym in sysSym_arr
			...
			# ! filter enries of preparation dictionary where capacity variable cannot exist, because there is no corresponding expansion variable 
			...
			# ! removes variables for retrofitting and grouped capacity for cases where no start capacity can exist
			for grpSym in filter(x -> occursin("grp",lowercase(string(x))),collect(keys(prepSys_dic[sys][sSym])))
				...
				filterZero_df = filter(x -> x.var != AffExpr(),grpCapa_tup.resi)

				# determine which combinations of disSup and disSup_last are possible
				if Symbol(:exp,type_sym) in keys(prepSys_dic[sys][sSym])
					...
					if anyM.parts.tech[:wind].type != :emerging exp_df[!,:Ts_expSup] .= 0 end
					exEtr_df = joinMissing(select(filterZero_df,Not([:var])),unique(select(exp_df,Not([:Ts_exp]))),intCol(filterZero_df),:outer,Dict())
				else
					exEtr_df = select(filterZero_df,Not([:var]))
				end

				join_arr = type_sym == :Conv ? [:Ts_expSup,:R_exp,:Te] : ( type_sym == :Exc ? [:Ts_expSup,:R_from,:R_to,:Te] : [:Ts_expSup,:R_exp,:Te,:id])

				# remove capacity entries
				prepSys_dic[sys][sSym][grpSym] = (var = orderDf(exEtr_df), resi = filterZero_df)
				...
				allRetro_df = innerjoin(allRetro_df,exEtr_df,on =  intCol(exEtr_df))
