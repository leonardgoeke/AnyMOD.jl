...
function defineParameter(options::modOptions, report::Array{Tuple,1})
    ...
    parDef_dic[:stLvlRefMonte]     = (dim = (:Ts_dis, :Ts_expSup, :R_dis, :C, :Te, :id),      problem = :sub,  defVal = 0.0     , herit = (:Ts_expSup => :up, :C => :up, :Ts_dis => :avg_any, :R_dis => :sum_any, :Te => :up, :id => :up),                                             part = :cost, techPre = (preset = :carrierSt, mode = tuple()))
